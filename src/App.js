...
export default function App() {
  ...
  /* Challenge
	...
	Bu form yeterince sinir bozucu değil. Sizin göreviniz aşağıdakileri yaparak onu daha sinir bozucu hale getirmek:
		...
		1. Formun her elementi kontrol edilen bir element olmalıdır. Her bir element aşağıdaki 109. satırdaki formData state nesnesindeki 
    ilgili özellik tarafından kontrol edilmelidir. Elemanlar ve karşılık gelen özellikleri aşağıdaki gibidir:

				+------+----------------+--------------+-------------------+-------------------+
				| Line | Element Türü   | Value        | Name              | formData Özelliği |
				+------+----------------+--------------+-------------------+-------------------+
				| 123  | text input     | n/a          | firstName         | firstName         |
				+------+----------------+--------------+-------------------+-------------------+
				| 130  | text input     | n/a          | lastName          | lastName          |
				+------+----------------+--------------+-------------------+-------------------+
				| 137  | email input    | n/a          | email             | email             |
				+------+----------------+--------------+-------------------+-------------------+
				| 150  | radio input    | "yes"        | privacyResponse   | privacyResponse   |
				+------+----------------+--------------+-------------------+-------------------+
				| 161  | radio input    | "no"         | privacyResponse   | privacyResponse   |
				+------+----------------+--------------+-------------------+-------------------+
				| 173  | radio input    | "absolutely" | privacyResponse   | privacyResponse   |
				+------+----------------+--------------+-------------------+-------------------+
				| 191  | select         | "n/a         | rating            | rating            |
				+------+----------------+--------------+-------------------+-------------------+
				| 206  | checkbox input | "n/a"        | marketingResponse | marketingResponse |
				+------+----------------+--------------+-------------------+-------------------+
				
		2. Gerçek bilgilerinizi girmeye çalışarak ve soruları kullanıcıların normalde yapacağı şekilde yanıtlayarak kodunuzu test edin. Önceki 
    görevi doğru bir şekilde tamamladıysanız, 71. satırın üzerindeki kodlar sayesinde tüm yanıtlarınız mizahi bir şekilde engellenmelidir. 
	
	İpucu: Formla etkileşim halindeyken, formla ne *olması gerektiğini* (ve form kontrol edilseydi ne olacağını) anlamak istiyorsanız: 
